...
export async function POST(req: Request, res: Response) {
  try {
    ...
    const createChaptersSchema = yup.object({
      title: yup.string().required(),
      units: yup.array(yup.string().required()).min(1),
    });
    const { title, units } = await createChaptersSchema.validate(body);
    type outputUnits = {
      title: string;
      chapters: {
